...
# Ye tokens ka deposit kahi bhi available nahi hai (manually confirm kiya
# gaya) - inke alerts kabhi nahi aayenge, chahe price gap kitna bhi ho
BLACKLISTED_TOKENS = {"ACX", "RAVE", "BLUAI", "ONE", "COTI"}
...
# Order-book depth check: itna USD value kam se kam available hona chahiye
# us exact bid/ask price par (warna trade karte hi price move ho jayega -
# slippage - aur genuine profit nahi milega). Agar size data hi na mile
# kisi exchange se, to check skip hota hai (block nahi karte).
MIN_ORDER_SIZE_USD = 30
...
CEX_LIST = [
    ...
    "coinex",
    "blofin", "deepcoin", "digifinex", "toobit",
    "weex", "xt",
    "whitebit", "bitrue", "exmo", "hitbtc",
    "p2b", "cryptocom", "bigone",
    "cex", "bydfi", "bitkan", "hashkey",
    # naye add kiye - jo ccxt me nahi honge wo automatically skip ho jayenge
    "famex", "btc", "latoken", "websea", "azbit", "webot",
    "cointr", "zoomex", "niza", "bitcointry", "bit2me", "bitunix",
    "grovex", "coinph", "zaif", "orbix", "bitdelta", "orangex", "safebit",
    "hibt", "kcex", "levex", "pionex",
]
...
def fetch_all_tickers(ex_id, exchange):
    ...
    for symbol, t in tickers.items():
        ...
        if base_check in BLACKLISTED_TOKENS:
            continue
        ...
        # Order-book depth - kitna quantity available hai is exact price par
        # (slippage check ke liye, sirf 24h total volume kaafi nahi hota)
        bid_size = safe_float(t.get("bidVolume"))
        ask_size = safe_float(t.get("askVolume"))
...
def has_enough_depth(price, size):
    """
    Agar size data available hai, check karo ki us price par kam se kam
    MIN_ORDER_SIZE_USD ka order pada hai ya nahi. Size data na ho to
    "unknown" maan ke pass hone dete hain (block nahi karte).
    """
    if not size or size <= 0:
        return True  # data nahi hai, check skip
    return (price * size) >= MIN_ORDER_SIZE_USD


def find_gaps_for_symbol(quotes_by_exchange, threshold_percent):
    """
    quotes_by_exchange = {exchange_id: {"bid": x, "ask": y, "bid_size":,
    "ask_size":}} for ONE symbol.
    ...
    check karta hai - raw gap "genuine" profit nahi hota. Order-book depth
    bhi check karta hai - agar us price par kaafi quantity nahi hai to
    slippage se profit khatam ho jayega, aisa gap skip karte hain.
    """
    gaps = []
    for ex_a, ex_b in combinations(quotes_by_exchange.keys(), 2):
        ...
        if q_a["ask"] > 0 and q_b["bid"] > 0:
            ...
            depth_ok = (
                has_enough_depth(q_a["ask"], q_a.get("ask_size"))
                and has_enough_depth(q_b["bid"], q_b.get("bid_size"))
            )
        ...
        if q_b["ask"] > 0 and q_a["bid"] > 0:
            ...
            net_profit_percent = gap_percent - ESTIMATED_ROUND_TRIP_FEE_PERCENT
            depth_ok = (
                has_enough_depth(q_b["ask"], q_b.get("ask_size"))
                and has_enough_depth(q_a["bid"], q_a.get("bid_size"))
            )
            if (0 < gap_percent <= MAX_SANE_GAP_PERCENT
                    and gap_percent >= threshold_percent
                    and net_profit_percent >= MIN_NET_PROFIT_PERCENT
...
def main():
    ...
    while True:
        ...
        # Blacklisted tokens (jinka deposit kahi bhi nahi hai) hata do -
        # sab sources (ccxt + manual) ke liye ek hi jagah
        for symbol in list(price_map.keys()):
            base = symbol.split("/")[0]
